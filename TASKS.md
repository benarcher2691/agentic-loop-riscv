...
- [ ] **Cycle counter, part 1: the counter and its CSR reads.** In `Processor`: a 64-bit free-running cycle counter (`reg [63:0] cycles`, +1 every `clk`, cleared by `!resetn`). Decode `isSYSTEM` with `funct3 == 3'b010` (CSRRS) and `rs1Id == 0` as a CSR read: `Iimm[11:0] == 12'hC00` returns `cycles[31:0]`, `12'hC80` returns `cycles[63:32]`; the value is written to `rd` and PC advances like any other instruction. Every other `isSYSTEM` encoding still halts. Encodings for the bench: `csrrs rd, cycle, x0` = `32'hC0002073 | (rd << 7)`, `csrrs rd, cycleh, x0` = `32'hC8002073 | (rd << 7)` — add `RDCYCLE(rd)` / `RDCYCLEH(rd)` helper tasks *in the bench* (do not edit `lib/`). Bench `tb/cycle_tb.v`: two reads separated by a known instruction sequence differ by exactly 3 × instructions (+1 per load); `cycleh` reads 0; `x0` as destination writes nothing; `EBREAK` still halts. Keep it small; the demo program is the next task. <!-- model: openrouter/z-ai/glm-5.3 -->
