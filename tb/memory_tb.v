...
// with hold while low. Words 0..32 hold the SOC's demo program (UART banner
// + one LED walk + echo loop, 30 code words + 3 message words), generated by
// the assembler tasks in rtl/memory.v; the bench re-assembles the identical
// program with the lib macros and cross-checks (soc_tb.v additionally checks
// the words against hand-assembled constants). Words 33..255 are filled by
// the bench so the whole 256-word address space can be exercised.
module memory_tb;
  `include "check.vh"
  `WATCHDOG(1_000_000)
...
`endif
  ...
  // Fill words 33..255 so the whole address space is readable (the ROM's own
