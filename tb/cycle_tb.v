...
// Cycle counter, part 1: a 64-bit free-running counter in Processor and its
// CSR read. csrrs rd, cycle, x0 (0xC00) returns the low 32 bits, csrrs rd,
// cycleh, x0 (0xC80) returns the high 32 bits (RV32I RDCYCLE/RDCYCLEH); every
// other SYSTEM encoding still halts.
//
...
//       (the high word is still 0 this early — nothing has wrapped).
...
module cycle_tb;
  ...
  initial begin
    ...
    `CHECK_EQ(dut.RegisterBank[7], 32'd0, "cycleh read returns 0 (high word still 0)")
    ...
    // ==== C6: 64-bit rollover — the high word carries when the low wraps ====
    // Halt at EBREAK so the FSM is frozen but the counter free-runs, then
    // deposit the pair two short of a low-word wrap and clock across it.
    memPC = 0;
    EBREAK();
    endASM();
    fillEbreak;
    startRun;
    waitHalt;
    `CHECK_EQ(dut.PC, 32'd0, "halted at C6's EBREAK");
    dut.cycles = 32'hFFFFFFFE;
    dut.cycleh = 32'd5;
    @(posedge clk); #1;   // FFFFFFFE -> FFFFFFFF, no carry yet
    `CHECK_EQ(dut.cycles, 32'hFFFFFFFF, "low word reached all-ones")
    `CHECK_EQ(dut.cycleh, 32'd5,        "high word unchanged before the wrap")
    @(posedge clk); #1;   // FFFFFFFF -> 0, high word carries 5 -> 6
    `CHECK_EQ(dut.cycles, 32'd0, "low word wrapped to 0")
    `CHECK_EQ(dut.cycleh, 32'd6, "high word incremented on the low-word wrap")
    @(posedge clk); #1;   // 0 -> 1, no further carry
    `CHECK_EQ(dut.cycles, 32'd1, "low word counting again after the wrap")
    `CHECK_EQ(dut.cycleh, 32'd6, "high word holds after a single carry")

    // ==== C7: RDCYCLEH reads the real high word (not a hardwired 0) ====
    //   word 0: RDCYCLEH(x7)   samples the high word at its EXECUTE
    //   word 4: RDCYCLE(x5)    low word, EXECUTE = 2 + 3 = 5 (as in C1/C3)
    //   word 8: EBREAK
    // The high word is deposited nonzero right after reset release; the low
    // word starts at 0 and never wraps in this short run, so the high word
    // holds its deposited value and RDCYCLEH must return exactly it.
    memPC = 0;
    RDCYCLEH(x7);
    RDCYCLE(x5);
    EBREAK();
    ...
    resetn = 0;
    repeat (3) begin @(posedge clk); #1; end
    `CHECK_EQ(dut.PC, 32'd0, "PC held at 0 during reset (C7)")
    resetn = 1;
    dut.cycleh = 32'h0000002A;   // 42 into the high word before it counts
    waitHalt;
    `CHECK_EQ(dut.PC, 32'd8, "halted at C7's EBREAK")
    `CHECK_EQ(dut.RegisterBank[7], 32'h0000002A,
              "RDCYCLEH returns the high word (0x2A), proving it is not a constant 0")
